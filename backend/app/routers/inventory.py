...
from app.db.models import InventoryItem, InventoryAuditLog, AuditLogAction, InventoryCategory, ItemLocationStock, Location
...
async def get_full_inventory_item(item_id: int, db: AsyncSession) -> InventoryItem:
    """Načte položku se VŠEMI potřebnými vnořenými vztahy, včetně lokací a stromu kategorií."""
    stmt = (
        select(InventoryItem)
...
async def create_inventory_item(
    ...
    log = InventoryAuditLog(
        ...
        action=AuditLogAction.created, details=f"Položka '{item.name}' byla vytvořena s nulovým stavem."
    )
    db.add(log)
    await db.commit()
...
@router.get("/by-ean/{ean}", response_model=InventoryItemOut, summary="Získání položky podle EAN kódu")
...
@router.get("/{item_id}", response_model=InventoryItemOut, summary="Získání detailu konkrétní skladové položky")
...
@router.post("/{item_id}/upload-image", response_model=InventoryItemOut, summary="Nahrání obrázku k položce")
async def upload_inventory_item_image(
    ...
    log = InventoryAuditLog(
        item_id=item.id, user_id=user_id, company_id=company_id,
        action=AuditLogAction.updated, details=f"Obrázek změněn z '{original_image_url}' na '{item.image_url}'."
    )
    db.add(log)
    await db.commit()
    return await get_full_inventory_item(item.id, db)

@router.patch("/{item_id}", response_model=InventoryItemOut, summary="Aktualizace skladové položky (pouze pro adminy)")
...
@router.delete("/{item_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Smazání skladové položky")
async def delete_inventory_item(
    ...
    if total_stock > 0:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, f"Cannot delete item with stock ({total_stock} pcs). Please withdraw all items first.")
