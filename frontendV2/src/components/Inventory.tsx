...
const Inventory: React.FC<InventoryProps> = ({ companyId }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Rozšířeno o manufacturer a supplier
    return itemsToFilter.filter(item =>
      item.name.toLowerCase().includes(lowercasedTerm) ||
      item.sku.toLowerCase().includes(lowercasedTerm) ||
      (item.manufacturer && item.manufacturer.toLowerCase().includes(lowercasedTerm)) ||
              ...
              {filteredItems.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    {/* ZOBRAZENÍ VÝROBCE */}
                    <td className="px-5 py-4 border-b border-slate-200 text-sm">
                      <p className="text-slate-700 whitespace-no-wrap">{item.manufacturer || '-'}</p>
                      {/* Dodavatele zobrazíme menším písmem pod výrobcem, abychom šetřili místo na šířku */}
                      {item.supplier && <p className="text-xs text-slate-400">Dod: {item.supplier}</p>}
                    </td>

                    <td className="px-5 py-4 border-b border-slate-200 text-sm"><p className="text-slate-900 whitespace-no-wrap">{item.sku}</p></td>

                    {/* ... (zbytek řádku: Množství, Cena, Kategorie, Akce... beze změny) */}
                    <td className="px-5 py-4 border-b border-slate-200 text-sm">
