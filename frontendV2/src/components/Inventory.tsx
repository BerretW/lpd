...
// Helper component for rendering category options recursively
...
const Inventory: React.FC<InventoryProps> = ({ companyId }) => {
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    try {
        ...
        const promises: [Promise<InventoryItem[]>, Promise<CategoryOut[]>?, Promise<LocationOut[]>?] = [api.getInventoryItems(companyId, categoryToFilter)];
        
        if (isInitial) {
            promises.push(api.getCategories(companyId));
            if (!isAdmin) {
                promises.push(api.getMyLocations(companyId));
            }
        }
        
        const [itemData, categoryData, accessibleLocsData] = await Promise.all(promises);
  ...
  }, [companyId, isAdmin]); // Rerun if user somehow changes role, though this is unlikely.
  ...
  const filteredItems = useMemo(() => {
    ...
    // For non-admins, filter to items that exist in their accessible locations, regardless of quantity.
    if (!isAdmin && accessibleLocationIds) {
      itemsToFilter = items.filter(item => 
        item.locations.some(loc => accessibleLocationIds.has(loc.location.id))
      );
    }
  ...
  const handleSave = async (formData: Partial<InventoryItem>) => {
      try {
          if (editingItem) {
              await api.updateInventoryItem(companyId, editingItem.id, formData);
          } else {
              await api.createInventoryItem(companyId, formData);
          }
          closeFormModal();
          await fetchData(false); // Refetch items after save
      } catch (error) {
  ...
  const handleMovementSave = async () => {
    ...
    await fetchData(false); // Refetch data to see updated quantities
  }

  if (loading && items.length === 0) return <div className="p-8">{t('inventory.loading')}</div>;
  ...
  return (
    <div className="p-8">
      ...
      {view === 'items' ? (
                ...
                {filteredItems.length > 0 ? (
                        ...
                        return (
                          <tr key={item.id} className="hover:bg-slate-50">
                            <td className="px-5 py-4 border-b border-slate-200 text-sm">
                              <p className="text-slate-900 whitespace-no-wrap flex items-center">
                                  {item.name}
                                  {isAdmin && item.is_monitored_for_stock && <Icon name="fa-bell" className="ml-2 text-blue-500" title={t('inventory.monitored', { threshold: item.low_stock_threshold })} />}
                              </p>
                            </td>
                            <td className="px-5 py-4 border-b border-slate-200 text-sm">
                              <p className="text-slate-900 whitespace-no-wrap">{item.sku}</p>
                            </td>
                            <td className="px-5 py-4 border-b border-slate-200 text-sm">
                            ...
                            <td className="px-5 py-4 border-b border-slate-200 text-sm">
                              <p className="text-slate-900 whitespace-no-wrap">{item.category?.name || '-'}</p>
                            </td>
                            <td className="px-5 py-4 border-b border-slate-200 text-sm">
                               {isAdmin && (
                                    <div className="flex space-x-2">
                                        ...
                                        <Button variant="secondary" onClick={() => setAdjustingItem(item)}>
                                            {t('inventory.stockOperations')}
                                        </Button>
                                    </div>
                               )}
                            </td>
                    ...
                    <tr>
                        <td colSpan={6} className="text-center py-12 text-slate-500">
                            ...
                            <p>
                              {items.length > 0 ? t('inventory.noItemsFound') : t('inventory.noItemsExist')}
                            </p>
                        </td>
                    </tr>
                )}
        ...
        <LocationManager companyId={companyId} />
      ) : (
          <CategoryManager companyId={companyId} />
      )}

      {isFormModalOpen && (
        <Modal title={editingItem ? "Upravit položku" : "Nová skladová položka"} onClose={closeFormModal}>
            <InventoryForm
                onSave={handleSave}
